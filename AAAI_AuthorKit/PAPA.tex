...
Each thread begins by attempting to extract an element $s\in OPEN$ which is safe for expansion.Each time a thread finds a safe $s$, it performs an expansion as described in Algorithm \ref{alg:expand}. The search terminates once the goal is safe for expansion.
