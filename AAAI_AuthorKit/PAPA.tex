...
\ELSIF{$\epsilon$ decreased since $g_p(s')$ was last seen}
\STATE $g_p(s') := \max(g_p(s') - (\epsilon_{old}-\epsilon)c(bp(s'),s'),$
\STATE \qquad\qquad\qquad$g(s') + (\epsilon-1)c(bp_p(s'),s'))$
\ENDIF
...
Fix a minimum-cost path $\pi$ from $s_{start}$ to $s$ such that $bp(s_j) = s_{j-1}$ whenever $g(s_j) = g^*(s_j)$. Let $s'$ be the first node on it which is in $OPEN \cup BE$. There are two cases to consider, depending on whether $\pi$ has a $CLOSED$ state before $s'$.

If so, then every state between it and $s'$ is also in $CLOSED$, since $s'$ is the first state on $\pi$ to be in $OPEN \cup BE$. In particular, let $s_p\in CLOSED$ be the predecessor of $s'$ on $\pi$. By the induction hypothesis, $g(s_p) \le \epsilon g^*(s_p)$. Therefore,
\[g_p(s') \le g(s_p) + \epsilon c(s_p,s') \le \epsilon(g^*(s_p) + c(s_p,s')) = \epsilon g^*(s').\]

In the other case, $\pi$ has no $CLOSED$ state before $s'$. Since $FROZEN \subset CLOSED$, it follows that $s'$ is the first state on $\pi$ to lie in $OPEN\cup BE\cup FROZEN$. By Lemma \ref{lem:opt}, $g(s') = g^*(s')$. Furthermore, $g_p(s') \le \epsilon g(s')$. Therefore,
