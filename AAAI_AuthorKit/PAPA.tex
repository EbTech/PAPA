...
\item $s\ne s_{start} \Rightarrow g(s) + (\epsilon-1)c_l \le g_p(s) \le \epsilon g(s)$
...
TODO: since Lemma \ref{lem:indep} fails when $s'=s_{start}$, if $s_{start} \in OPEN\cup BE$, then $g_{front}$ will have to be initialized to $\epsilon h(s_{start},s)$ instead of $\infty$ in order for Lemma \ref{lem:bound} to hold.
...
Let $\pi = \langle s_0,s_1,\ldots,s_N \rangle$ be a minimum-cost path from $s_0 = s_{start}$ to $s_N = s\in OPEN\cup BE$. Choose the minimum $i$ such that $s_i\in OPEN\cup BE$. If $i < 2$, then
\[g_p(s_i) \le \epsilon g(s_i) = g^*(s_i)\]
