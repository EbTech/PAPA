...
\STATE among $s\in OPEN$ such that $g(s) \le bound(s)$, choose one with the smallest $f(s)$ and LOCK $s$
...
\begin{lemma}
\label{lem:prop}
At all times, the following properties hold:
\begin{itemize}
\item $bp(s) = argmin_{s'}\{v(s') + c(s',s)\}$
\item $g(s) = v(bp(s)) + c(bp(s),s)$
\item $s\in OPEN\cup BE\cup FROZEN \Rightarrow g(s) < v(s)$
\item $s\notin OPEN\cup BE\cup FROZEN \Rightarrow g(s) = v(s)$
\item $s\neq s_{start} \Rightarrow g_p(s) \ge g(s) + (\epsilon-1)c_l$
...
Let $\pi = \langle s_0,s_1,\ldots,s_N \rangle$ be a minimum-cost path from $s_0 = s_{start}$ to $s_N = s$, such that $bp(s_j) = s_{j-1}$ whenever $g(s_j) = g^*(s_j)$. Let $s_i$ be the first node in $\pi \cap(OPEN \cup BE)$. There are three cases to consider. First, suppose $s_{i-1}\in CLOSED$. Then
\[g_p(s_i) \le \epsilon(g^*(s_{i-1}) + c(s_{i-1},s_i)) = \epsilon g^*(s_i)\]

Now suppose $s_j\in CLOSED$ for some $j<i-1$, and that $s_k \notin OPEN\cup BE\cup CLOSED$ for all $j<k<i$. Then $v(s_j) \le \epsilon g^*(s_j)$ by the induction hypothesis. Since the in-between states are consistent, $g(s_i) \le v(s_j) + c^*(s_j,s_i) \le \epsilon g^*(s_j) + c^*(s_j,s_i)$. Therefore,
\begin{eqnarray*}
g_p(s_i) &\le& g(s_{i}) + 2(\epsilon-1)c_l
\\&\le& \epsilon g^*(s_j) + c^*(s_j,s_i) + 2(\epsilon-1)c_l
\\&=& \epsilon(g^*(s_i) - c^*(c_j,s_i)) + c^*(s_j,s_i) + 2(\epsilon-1)c_l
\\&=& \epsilon g^*(s_i) + (\epsilon-1)(2c_l - c^*(s_j,s_i))
\\&\le& \epsilon g^*(s_i)
\end{eqnarray*}

Finally, suppose $s_j \notin OPEN\cup BE\cup CLOSED$ for all $j<i$. By consistency, it must be the case that $g(s_i) = g^*(s_i)$. Furthermore, since $s_{i-1} \notin CLOSED$ even though $bp(s_i) = s_{i-1}$, $s_i$ could not have been added to $OPEN$ by an expansion. Instead, it must have been transferred there from $FROZEN$. Therefore,
\[g_p(s_i) \le \epsilon g(s_i) = \epsilon g^*(s_i)\]

In all three cases,
\[g_p(s_i) + \epsilon h(s_i,s) \le \epsilon\left(g^*(s_i) + c^*(s_i,s)\right) = \epsilon g^*(s).\]
