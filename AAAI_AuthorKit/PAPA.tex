...
Let $\pi = \langle s_0,s_1,\ldots,s_N \rangle$ be a minimum-cost path from $s_0 = s_{start}$ to $s_N = s\in OPEN\cup BE$
%, such that $bp(s_j) = s_{j-1}$ whenever $g(s_j) = g^*(s_j)$.
There are two cases to consider, depending on whether there exists $s_i\in\pi$ such that $s_i\in CLOSED\setminus BE$.
...
\[g_p(s_{i+1}) \le \epsilon g^*(s_i) + \epsilon c(s_i,s_{i+1}) = \epsilon g^*(s_{i+1})\]

One the other hand, suppose there is no $s_i\in CLOSED\setminus BE$. Choose the minimum $i$ such that $s_i\in OPEN\cup BE$. Then $s_j\notin OPEN\cup CLOSED$ for all $j<i$, which by Lemma \ref{lem:prop} implies $g(s_j)=v(s_j)$ and $g(s_i) = \sum_{j=1}^i c(s_{j-1},s_j) = g^*(s_i)$. Therefore,
