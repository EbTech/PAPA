...
is a recent approach which expands each state at most once
...
We assume the distances can be estimated by a \textbf{goal-consistent heuristic} $h$, meaning $h(s',s_{goal}) - h(s,s_{goal})\le h(s',s)\le c^*(s',s)$ for all $s,s'$. This is a stronger condition than admissibility, but weaker than pairing admissibility with the triangle inequality. Note that if $h_g$ is a consistent distance-to-goal estimate, then the 2-argument heuristic $h(s',s) = \max(0,\,h_g(s') - h_g(s))$ is admissible with triangle inequality.
...
In particular, instantiate $s'$ to be the first state on the optimal path from $s_{start}$ to $s$ which lies on the search frontier. Inductively assuming the predecessor of $s'$ along that path was expanded with an $\epsilon$-optimal $g$-value, then we also have $g(s') \le \epsilon g^*(s')$. Thus,
\[bound(s) \le g(s') + \epsilon h(s',s) \le \epsilon (g^*(s') + h(s',s)) \le \epsilon g^*(s).\]
...
\begin{algorithm}
\caption{wA* without initialization details}
\label{alg:wastar}
\begin{algorithmic}
\WHILE{$g(s_{goal}) > bound(s_{goal})$}
\STATE remove $s\in OPEN$ with $g(s) \le bound(s)$
